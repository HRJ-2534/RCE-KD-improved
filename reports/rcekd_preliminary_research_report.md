...
我们还检查了对非均匀采样 CE 进行重要性校正的可能性。双边 log-Q 校正可将平均梯度余弦从 0.612 提高到 0.976，但使方差扩大约 73 倍；使用独立样本拟合的收缩系数 \(0.101\) 虽然将梯度 MSE 降低 6.10%，正式训练后 NDCG@20 仍由 AM-RCE-KD 的 0.01969 降至 0.01912。该结果进一步表明，逼近完整 CE 梯度并不等价于改善 Top-K 排序，最终方案应保留当前有益的采样偏置。
...
当前结果属于预研性质，主要限制是仅在 CiteULike、BPR→BPR 和单个随机种子上完成。正式结论仍需补充至少 3 个随机种子，并在 Gowalla 或 Yelp、以及异构 Teacher/Student 上验证均值、标准差和泛化能力。因此，本文可以声称方案已经表现出明确可行性，但暂不声称其具有跨数据集的稳定优势。
